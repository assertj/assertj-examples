...
public class IterableAssertionsExamples extends AbstractAssertionsExamples {
	...
	public void iterable_assertions_with_custom_comparator_examples() {
		...
		// ... but if we compare only race name Sauron is in fellowshipOfTheRing
		// because he's a Maia like Gandalf.
		assertThat(fellowshipOfTheRing).usingElementComparator(raceNameComparator).contains(sauron);

		// note that error message mentions the comparator used to better
		// understand the failure
		// the message indicates that Sauron were found because he is a Maia
		// like Gandalf.
		try {
	...
	public void iterable_assertions_on_extracted_values_example() {
		...
		// same assertions but extracting properties fluently done outside
		// assertions

		// extract simple property values having a java standard type
		assertThat(extractProperty("name", String.class).from(fellowshipOfTheRing)).contains("Boromir", "Gandalf",
				"Frodo").doesNotContain("Sauron", "Elrond");

		// same extraction with an alternate syntax
		assertThat(extractProperty("name").ofType(String.class).from(fellowshipOfTheRing)).contains("Boromir",
				"Gandalf", "Frodo", "Legolas").doesNotContain("Sauron", "Elrond");

		// extracting property works also with user's types (here Race)
		assertThat(extractProperty("race").from(fellowshipOfTheRing)).contains(HOBBIT, ELF).doesNotContain(ORC);

		// extract nested property on Race
		assertThat(extractProperty("race.name").from(fellowshipOfTheRing)).contains("Hobbit", "Elf").doesNotContain(
				"Orc");
	...
	public void iterable_type_safe_assertion_example() {
		// just to show that containsAll can accept subtype of is not bounded to
		// Object only
		Collection<Ring> elvesRings = newArrayList(vilya, nenya, narya);
