...
public class StringAssertionsExamples extends AbstractAssertionsExamples {
	...
	public void string_assertions_examples() {
		...
		final String bookDescription = "{ 'title':'Games of Thrones', 'author':'George Martin'}";
		...
		try {
			assertThat(bookDescription).containsSubsequence("{", "title", "author", "Games of Thrones", "}");
		} catch (final AssertionError e) {
	...
	public void string_assertions_with_custom_comparison_examples() {
		...
		assertThat("Frodo").usingComparator(this.caseInsensitiveStringComparator).startsWith("fr").endsWith("ODO");

		// All assertions made after usingComparator(caseInsensitiveStringComparator) are based on the given comparator ...
		assertThat("Frodo").usingComparator(this.caseInsensitiveStringComparator).contains("fro").doesNotContain("don");
		...
		assertThat("Frodo").usingComparator(this.caseInsensitiveStringComparator).doesNotMatch("..O.O");
		try {
			// failed assertion with specific comparator mention the comparison strategy to help interpreting the error
			assertThat("Frodo").usingComparator(this.caseInsensitiveStringComparator).startsWith("FROO");
		} catch (final AssertionError e) {
	...
	public void charSequence_subclasses_assertions_examples() {
		...
		final StringBuilder stringBuilder = new StringBuilder("Frodo");
		...
		final Comparator<CharSequence> caseInsensitiveComparator = new Comparator<CharSequence>() {
		...
		// StringBuilder have same assertions as String
		final StringBuffer stringBuffer = new StringBuffer("Frodo");
	...
	public void xml_assertions_examples() {
		...
		final String expectedXml = "<rings>\n" +
				"  <bearer>\n" +
				"    <name>Frodo</name>\n" +
				"    <ring>\n" +
				"      <name>one ring</name>\n" +
		...
		final String xmlWithNewLine = "<rings>\n" +
				"<bearer>   \n" +
				"  <name>Frodo</name>\n" +
				"  <ring>\n" +
				"    <name>one ring</name>\n" +
		...
		final String unexpectedXml = "<rings>\n" +
				"<bearer>   \n" +
				"  <name>Sauron</name>\n" +
	...
	public void switch_to_String_assertion() {
		final Object title = "Game of Thrones";
	...
	public void multine_collection_formatting() {
		final String[] greatBooks = array("A Game of Thrones", "The Lord of the Rings", "Assassin's Apprentice  ....",
				"Disc World");
		final String smartFormat = STANDARD_REPRESENTATION.toStringOf(newArrayList(greatBooks));
