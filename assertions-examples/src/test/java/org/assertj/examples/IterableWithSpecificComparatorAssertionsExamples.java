...
public class IterableWithSpecificComparatorAssertionsExamples extends AbstractAssertionsExamples {
	...
	public void iterable_assertions_with_element_comparator_examples() {
		...
		assertThat(this.fellowshipOfTheRing).contains(this.gandalf).doesNotContain(this.sauron);
		// ... but if we compare only race name Sauron is in fellowshipOfTheRing because he's a Maia like Gandalf.
		assertThat(this.fellowshipOfTheRing).usingElementComparator(this.raceNameComparator).contains(this.sauron);
		...
		try {
			assertThat(newArrayList(this.gandalf, this.sam)).usingElementComparator(this.raceNameComparator).doesNotContain(this.sauron);
		...
		assertThat(this.fellowshipOfTheRing).doesNotHaveDuplicates();
		assertThat(newArrayList(this.sam, this.gandalf)).usingElementComparator(this.raceNameComparator)
				.doesNotHaveDuplicates()
				.isEqualTo(newArrayList(this.frodo, this.gandalf));
		try {
			assertThat(newArrayList(this.sam, this.gandalf, this.frodo)).usingElementComparator(this.raceNameComparator)
					.doesNotHaveDuplicates();
		} catch (final AssertionError e) {
	...
	public static class Dude {
		...
		Dude(final String name, final double height) {
			this.name = name;
			this.height = height;
		}
		...
		public String toString() {
			return String.format("Dude [name=%s, height=%s]", this.name, this.height);
		}
	...
	public void iterable_assertions_with_specific_comparator_by_element_field_or_type_examples() {
		...
		final Dude dude = new Dude("Frodo", 1.2);
		final Dude tallerDude = new Dude("Frodo", 1.3);

		final Comparator<Double> closeEnough = new AtPrecisionComparator<>(0.5);

		final Dude[] hobbits = new Dude[] { dude };
		...
		final Dude reallyTallDude = new Dude("Frodo", 1.9);
		try {
			assertThat(asList(dude)).usingComparatorForElementFieldsWithNames(closeEnough, "height")
					.usingFieldByFieldElementComparator()
	...
	public void iterable_assertions_with_recursive_field_by_field_element_comparator_examples() {
		final Dude jon = new Dude("Jon", 1.2);
		final Dude sam = new Dude("Sam", 1.3);
		jon.friend = sam;
		sam.friend = jon;

		final Dude jonClone = new Dude("Jon", 1.2);
		final Dude samClone = new Dude("Sam", 1.3);
		...
		assertThat(jon).isEqualToComparingFieldByFieldRecursively(jonClone);
	}
